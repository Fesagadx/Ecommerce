      ...
      <Row className="product-detail-images">
        <Col className="product-img" xs={12} lg={5}>
          <Carousel activeIndex={activeIndex} onSelect={handleSelect}>
            {
              detail?.productImgs?.map((element, index) => (
                <Carousel.Item key={index} >
                  <img className="product-centered-img" src={`${element}`} alt={`img ${index}`}/>
                </Carousel.Item>
              ))
            }
          </Carousel>
          ...
          <div className="product-miniature-container">
            {
              detail?.productImgs?.map((element, index) => (
                <img key={index} src={`${element}`} alt={`img ${index}`} className={`product-miniature ${activeIndex === index ? 'active' : ''}`} onClick={() => setActiveIndex(index)}/>
              ))
            }
          </div>
        </Col>
      ...
      <Container className="product-equal col-12">
        {
          similarItems.map((element, index) =>
            <Card as={Link} to={`/products/${element.id}`} key={index} className='product-equal-card mx-3 mb-3'>
              ...
              <Card.Header >
                <Card.Img className='similar-items-img similar-items-imgOne' variant="top" src={`${element.productImgs[0]}`} />
                <Card.Img className='similar-items-img similar-items-imgTwo' variant="top" src={`${element.productImgs[1]}`} />
              </Card.Header>
